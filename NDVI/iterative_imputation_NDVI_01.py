...
#imputazione dei dati considerati come input del modello
...
mse_iniziale = mean_squared_error(NDVI_measured_s['NDVI'], d_s['NDVI_completed'].loc[NDVI_index])
print("Punteggio del mlp nella predizione di NDVI: ", score)
print("Errore quadratico medio tra NDVI predetto e misurato: ", mse_iniziale)
...
while(d_s['NDVI'].count() < 939):
    ...
    if score < score_min_d:
         score_min_d = score
    if score > score_max_d:
        score_max_d = score
    score_sum += score 
    score_average = score_sum/loop   
    ...
    print("mse del mlp:", mlp_mse)
    
    mlp_mae =  mean_absolute_error(y_test, NDVI_predicted)
    print("mae del mlp:", mlp_mae)
    
    # mlp_max_e =  max_error(y_test, NDVI_predicted)
    # print("max error del mlp:", mlp_max_e)
...
mse_finale = mean_squared_error(y_predicted, y_imputed)
...
d_s2 = d_s.loc[:, cols]
inversione_deficit = pd.DataFrame(scaler.inverse_transform(d_s2),columns=cols, index=d.index)

#salva punteggi
score_array = np.array([score_min_d,score_max_d,score_average,mse_iniziale,mse_finale])
score_array =np.reshape(score_array, (1,5))
d_score = pd.DataFrame(score_array, columns = ['r2_score_min_','r2_score_max_d','r2_score_average_deficit', 'mse_iniziale','mse_finale'],index = ['scores_ndvi_deficit'])
d_score.to_csv('deficit_score.csv', sep=';', decimal=',', index=True)

#salva imputazione su csv
NDVI_deficit = inversione_deficit.loc[:, 'NDVI']
Day_deficit = db_d.loc[:, 'Day']
NDVI_imputed_deficit = pd.concat([Day_deficit, NDVI_deficit], axis=1)
NDVI_imputed_deficit.to_csv('deficit_ndvi.csv', sep=';', decimal=',', index=True)

plt.scatter(d_index, inversione_deficit['NDVI'], c="red", alpha=0.2, s=10, label="NDVI Imputed")
...
plt.title("Imputed and Measured NDVI - Deficit")
plt.legend()
plt.show()

print("MSE di alcuni valori random di NDVI Imputati contro quelli Predetti dagli NDVI Misurati:", mse_finale)
...
f_s2 = f_s.loc[:, cols]
inversione_full = pd.DataFrame(scaler.inverse_transform(f_s2),columns=cols, index=f.index)


plt.scatter(f_index, inversione_full['NDVI'], c="red", alpha=0.2, s=10, label="NDVI Imputed")
plt.scatter(NDVI_index_f, NDVI_measured_f['NDVI'], c="blue", alpha=0.3, s=2, label="NDVI Measured")
# plt.scatter(empty_index, NDVI_imputed, c="green", alpha=0.2, s=15, label="NDVI SciKitImputed")
# plt.plot(f_index, f_s['ET0'], c="black", alpha=0.3, label="ET0")


#salva imputazione su csv
Day_full = db_f.loc[:, 'Day']
NDVI_full = inversione_full.loc[:, 'NDVI']
# NDVI_full = NDVI_full.join(NDVI_full_imputed)
NDVI_imputed_full = pd.concat([Day_full, NDVI_full], axis=1)
NDVI_imputed_full.to_csv('full_ndvi.csv', sep=';', decimal=',', index=True)
