...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb  9 16:43:45 2022

@author: Federico Amato
Dopo l'imputazione iterativa di NDVI si vede quanto i punti
imputati riescano a predire bene i punti misurati
"""
...
# Estrae i dati di NDVI e NDWI imputati
df_nd = et.make_dataframe(DATABASE_ND, columns=['NDVI-ML','NDWI-ML'],
                          method='drop',
                          drop_index=True)
...
# Lista dei giorni con imputazioni di ND
not_measures_days = [day for day in df_total.index if day not in measures_days]

# Per l'Addestramento si prendono tutte le features, fino a ET0 (compresa)
X_train = df_total.loc[not_measures_days,:'ET0']
# e i valori imputati degli Indici
y_train_ndvi = df_total.loc[not_measures_days,'NDVI-ML']
y_train_ndwi = df_total.loc[not_measures_days,'NDWI-ML']
# Per il Test si prendono invece i dati Misurati
...
fig.suptitle('NDVI Measured vs Prediction by Imputed data', **FIGFONT)
et.plot_axis(ax_ndvi, [measures_days, y_measured_ndvi, 'grey'],
             legend='NDVI measured',**FIG_PARAMS)
et.plot_axis(ax_ndvi, [measures_days, best_row['Prediction'], 'blue'], plot_type = 'scatter',
...
fig.suptitle('NDWI Measured vs Prediction by Imputed data', **FIGFONT)
et.plot_axis(ax_ndwi, [measures_days, y_measured_ndwi, 'grey'],
             legend='NDWI measured',**FIG_PARAMS)
et.plot_axis(ax_ndwi, [measures_days, y_predicted_ndwi, 'blue'], plot_type = 'scatter',
