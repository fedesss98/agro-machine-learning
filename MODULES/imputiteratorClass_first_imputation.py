...
class ImputeIterator:
    ...
    def fit(self, features, target, idx_maivisti=None, model=None):
        self.fitted = True
        self.X = features
        ...
        if idx_maivisti is not None:
            idx_visti = [idx for idx in target.index
                         if idx not in idx_maivisti]
            X_test = features.loc[idx_maivisti]
            y_test = target.loc[idx_maivisti]
            X_train = features.loc[idx_visti]
            y_train = target.loc[idx_visti]
        else:
            X_train, X_test, y_train, y_test = train_test_split(
                features.loc[target.index], target,
                test_size=test_size, random_state=58,
            )
        y_idx = np.sort(X_test.index)
        plt.plot(y_idx)
        plt.title(f"Index of Test data inside Imputeiterator ({len(y_idx)})")
        plt.show()
        X_test.iloc[:, 0].plot()
        plt.title("First Feature in Test")
        plt.ylim(0, 35)
        plt.grid()
        plt.show()
        ...
        plt.plot(X_test[:, 0])
        plt.title("First Feature in Test (scaled)")
        plt.ylim(-1.5, 1.5)
        plt.grid()
        plt.show()
        # Addestramento
...
# =============================================================================
    ...
    def __print_header(self):
        print(f'First MaiVisti Scores:\n'
              f'{"r2":7} {self.score_mv[0][0]:0.4}\n'
              f'{"rmse":7} {math.sqrt(self.score_mv[0][1]):0.4}\n'
              f'{"mbe":7} {self.score_mv[0][2]:0.4}\n'
              )
        if self.verbose:
            print(
                f"\nSCORES:\ni"
